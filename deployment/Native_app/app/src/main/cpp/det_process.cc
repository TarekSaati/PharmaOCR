...
void DetPredictor::Preprocess(const cv::Mat &srcimg, const int max_side_len) {
  ...
  cv::Mat img_fp;
  srcimg.convertTo(img_fp, CV_32FC3, 1.0 / 255.f);
...
DetPredictor::Postprocess(const cv::Mat srcimg,
  ...
  auto shape_out = output_tensor->shape();
    //LOGD("ocr cpp output tensor[%d] size {%d, %d, %d, %d}", 0, shape_out[0], shape_out[1], shape_out[2], shape_out[3]);
  ...
  for (int i = 0; i < out_size; i++) {
    ...
    cbuf[i] = static_cast<unsigned char>(outptr[i] * 255);
      //LOGD("Data pixel %d = %f", i, outptr[i]);
  }
  ...
  auto boxes = BoxesFromBitmap(pred_map, bit_map, Config);
    LOGD("Num of boxes = %d", boxes.size());
  std::vector<std::vector<std::vector<int>>> filter_boxes =
      FilterTagDetRes(boxes, 1, 1, srcimg);
