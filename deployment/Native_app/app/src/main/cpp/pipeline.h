...
class Pipeline {
public:
  Pipeline(const std::string &detModelDir, const std::string &classModelDir,
  ...
  bool Process_val(int inTextureId, int outTextureId, int textureWidth,
    ...
    static std::vector<std::string> getOCRResults();
...
private:
  ...
  std::vector<std::string> items_list_;
  std::shared_ptr<DetPredictor> detPredictor_;
  std::shared_ptr<ClassPredictor> classPredictor_;
